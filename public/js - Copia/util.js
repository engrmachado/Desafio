const BASE_URL = "http://localhost/site_curso/";
...
function uploadImg(input_file, img, input_path){
	...
	$.ajax({
		url: BASE_URL + "restrito/ajax_importar_imagem",
		dataType: "json",
		cache: false,
		contentType: false,
		processData: false,
		...
		success: function(response){
			...
			if(response["status"]){
				...
				input.path.siblings(".help-block").html(response["error"]);


			}
		},
		error: function(){
			img.atrr("src", src_before);
